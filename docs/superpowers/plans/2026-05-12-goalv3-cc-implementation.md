...
ls -la /Users/0xvox/Windburn/.gitignore
```
...
- Modify: `/Users/0xvox/Windburn/.gitignore` (add `.goal/` line if absent)
...
constraints: <hard non-negotiables>       # e.g. "no modify /Users/0xvox/.codex/"
...
role_boundary:
  ...
  must_not_take_over: codex /Users/0xvox/.codex/ (operator-locked), MUW autopilots

repo_anchor:
  cwd: /Users/0xvox/Windburn
  branch: main
...
Codex (GPT-5.5) running on Windburn / MUW sessions was observed self-dispatching `gsd-*` subagents in `spec → plan → review` sequences, and at every stage transition self-applying PASS/FLAG/BLOCK verdicts. Documented in `/Users/0xvox/.codex/memories/MEMORY.md` task group "Bounded Supervisor Review Gate v0" with concrete config:
...
- Codex source: `/Users/0xvox/.codex/memories/MEMORY.md` — task group "Bounded Supervisor Review Gate v0"
````
...
- The goal touches `/Users/0xvox/multica-ultimate-workbench/autopilots/` or `/Users/0xvox/.codex/` — these are operator-locked lanes; abort with operator-call
...
2. **Modify operator-locked lanes.** `/Users/0xvox/multica-ultimate-workbench/autopilots/` and `/Users/0xvox/.codex/` are frozen by operator directive. Any packet whose route touches these → OPERATOR_NEEDED immediately.
...
use goalv3-cc on goal "absorb-codex-into-cc": port codex chronicle + hooks + mcp + agent registry into CC analogs
  ...
  constraints:
    - DO NOT modify /Users/0xvox/.codex/ (operator-locked)
    - DO NOT modify /Users/0xvox/multica-ultimate-workbench/autopilots/ (operator-locked)
```
